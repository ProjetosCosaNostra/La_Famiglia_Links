...
async function getEvidence(env,id){
  return env.DB.prepare(`SELECT r.*,p.title_pt,p.marketplace_catalog_product_id,p.marketplace_identity_status
...
export async function onRequestPost({request,env}){
  ...
  const nextStatus='catalog_only';
  const reason='reconciliation_catalog_accepted_pending_listing_verification';
  await env.DB.batch([
    ...
    env.DB.prepare(`UPDATE products SET marketplace_catalog_product_id=?,marketplace_identity_status=?,marketplace_last_checked_at=NULL,marketplace_last_reason=?,updated_at=? WHERE id=?`).bind(before.candidate_catalog_product_id,nextStatus,reason,now,before.product_id),
    env.DB.prepare(`INSERT INTO admin_audit(action,entity_type,entity_id,actor,before_json,after_json) VALUES('accept_reconciliation','marketplace_reconciliation',?,'admin',?,?)`).bind(id,JSON.stringify(before),JSON.stringify({review_status:'accepted',catalog_product_id:before.candidate_catalog_product_id,identity_status:nextStatus}))
  ]);
  return json({ok:true,id,product_id:before.product_id,review_status:'accepted',catalog_product_id:before.candidate_catalog_product_id,identity_status:nextStatus});
}