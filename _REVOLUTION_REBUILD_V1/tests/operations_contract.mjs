...
async function testGetNormalization(){
  const env={BG_ADMIN_TOKEN:'t',WORKER:{fetch:async()=>Response.json({ok:true})},DB:{prepare(sql){return {all:async()=>sql.includes('FROM channel_state')?{results:[{channel:'instagram',policy_status:'INTERNAL_ALLOWED',configured:1,live_enabled:1},{channel:'site_daily',policy_status:'REVIEW_REQUIRED',configured:0,live_enabled:1}]}:{results:[]}}}}};
