...
class FakeDB{
  ...
  apply(sql,a){
    ...
    if(sql.startsWith('UPDATE products SET slug=')){
      const id=a[23],p=this.products.get(id);const keys=['slug','title_pt','title_en','title_es','brand','category','description_pt','status','affiliate_ready','featured','image_path','image_status','updated_at','marketplace_catalog_product_id','marketplace_expected_attributes_json','marketplace_identity_status','marketplace_title','marketplace_permalink','marketplace_status','marketplace_seller_id','marketplace_seller_level','marketplace_last_checked_at','marketplace_last_reason'];keys.forEach((k,i)=>p[k]=a[i]);p.market='BR';p.marketplace_site_id='MLB';return;
    }
