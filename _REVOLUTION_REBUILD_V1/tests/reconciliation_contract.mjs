...
class FakeDB{
  constructor(){
    this.products=new Map([
      ['bg-us-0007',{id:'bg-us-0007',title_pt:'Belle Angel',marketplace_catalog_product_id:null,marketplace_identity_status:'pending'}],
      ['bg-us-0006',{id:'bg-us-0006',title_pt:'Ruby Rose',marketplace_catalog_product_id:null,marketplace_identity_status:'pending'}],
      ['bg-us-0001',{id:'bg-us-0001',title_pt:'Lily',marketplace_catalog_product_id:null,marketplace_identity_status:'pending'}]
    ]);
  ...
  async batch(stmts){
    for(const s of stmts){const {sql,args:a}=s;
      ...
      else if(sql.startsWith('UPDATE products SET')){const p=this.products.get(a[4]);Object.assign(p,{marketplace_catalog_product_id:a[0],marketplace_identity_status:a[1],marketplace_last_reason:a[2]});}
    }
    return [];
  }
}
