...
const TABLES={
  ...
  clicks:["id","product_id","product_title","placement","clicked_at"]
};
...
export async function onRequestGet(context){
  ...
  try{
    const [products,revisions,archives,audit,clicks]=await Promise.all([
      ...
      rows(context.env.BG_DB,"SELECT * FROM outbound_clicks ORDER BY clicked_at ASC,id ASC")
    ]);
    return json({
      ok:true,
      schema:"blackgold-db-snapshot-v1",
      createdAt:new Date().toISOString(),
      counts:{products:products.length,revisions:revisions.length,archives:archives.length,audit:audit.length,clicks:clicks.length},
  ...
  try{
    const current=await context.env.BG_DB.prepare(
      `SELECT
        ...
        (SELECT COUNT(*) FROM outbound_clicks) AS clicks`
    ).first();
    ...
    const d=snap.data;
    const statements=[
      context.env.BG_DB.prepare("DELETE FROM outbound_clicks"),
    ...
    for(const row of d.clicks||[])statements.push(insertStatement(context.env.BG_DB,"outbound_clicks",TABLES.clicks,row));
