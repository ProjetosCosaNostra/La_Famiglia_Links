...
try{
  ...
  for(const imgX of [-20,-10,0,10]){
    for(const imgY of [-20,-10,0,10]){
      for(const scale of [.95,1.05,1.15,1.25]){
        const top=698,height=144,alpha=.20;
        const id=`show-compact-x${imgX}-iy${imgY}-s${String(scale).replace(".","_")}`;
        await inject(`
          .showcase-live .media img.bg-compact-wide{
        ...
        showcase.push({id,top,height,imgX,imgY,scale,alpha,profile:"compact-wide-only"});
      }
    }
  }
