...
def fetch(blob,size):
 ...
 q=urllib.request.Request(u,headers={"User-Agent":"BlackGold-Visual-V50/1","Accept":"application/vnd.github+json"})
 with urllib.request.urlopen(q,timeout=45) as res:x=json.loads(res.read().decode("utf-8"))
 if x.get("sha")!=blob or x.get("encoding")!="base64":raise RuntimeError("visual blob integrity failure")
 b=base64.b64decode(x["content"])
 if len(b)!=size:raise RuntimeError("visual blob size mismatch")
 return b
def atomic(path,data):
 path.parent.mkdir(parents=True,exist_ok=True);tmp=path.with_name(path.name+".bg50.tmp");tmp.write_bytes(data);os.replace(tmp,path)
def main():
 ...
 for name,(blob,size) in FILES.items():atomic(stage/name,fetch(blob,size))
 required=["data/products.json","data/guides.json","data/retailer-links.json"]
 missing=[p for p in required if not (stage/p).exists()]
 if missing:raise SystemExit("V5.0 dependencies missing: "+", ".join(missing))
 return 0
