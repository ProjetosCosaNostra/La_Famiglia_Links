...
def fetch(blob,size):
 ...
 q=urllib.request.Request(u,headers={"User-Agent":"BlackGold-Visual-V52/1","Accept":"application/vnd.github+json"})
 with urllib.request.urlopen(q,timeout=45) as r:x=json.loads(r.read().decode("utf-8"))
 if x.get("sha")!=blob or x.get("encoding")!="base64":raise RuntimeError("V5.2 blob integrity failure")
 b=base64.b64decode(x["content"])
 if len(b)!=size:raise RuntimeError("V5.2 blob size mismatch")
 return b
def atomic(path,data):
 path.parent.mkdir(parents=True,exist_ok=True);tmp=path.with_name(path.name+".bg52.tmp");tmp.write_bytes(data);os.replace(tmp,path)
def main():
 ...
 if missing:raise SystemExit("V5.2 dependencies missing: "+", ".join(missing))
